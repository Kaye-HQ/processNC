...
summariseRast <- function(files=files, startdate=NA, enddate=NA, ext=NA, var,
  ...
  if(overwrite==FALSE & file.exists(filename1)){
    ...
    if(class(files) %in% c("SpatRaster")){
      data <- files
    } else{
      if(length(files) > 1){
    ...
    index_time <- as.Date(paste0("01", zoo::as.yearmon(terra::time(data))), format="%d %b %Y")
    ...
    terra::time(avg) <- unique(index_time)
    avg <- terra::tapp(avg, index=as.numeric(floor(lubridate::month(terra::time(avg)))), fun=mean, na.rm=TRUE)
    ...
    if(filename2 != ""){
      cv <- terra::tapp(data, by=zoo::as.yearmon(terra::time(data)), fun=cv, name='months', na.rm=TRUE)
      cv <- terra::tapp(cv, by=function(x) as.numeric(floor(lubridate::month(terra::time(data)))), 
